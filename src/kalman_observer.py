...
# Not sure why but check feedback
def feedback_good(accel_power, accel_z, max_z):
    return (accel_z != np.NAN and accel_power > 0.5 and accel_z < max_z) # accel_power > 2.0

# Our Project observer
class KalmanBased_Observer(Kalman):
    def __init__(self, 
                 Q_mat, 
                 R_mat, 
                 initX, 
                 ...
                 B, 
                 H,
                 z_0 = None, # From here, Added from source: tcc_pose_observer.py
                 z_0_dot = 0.0,
                 dt = 0.005,
                 max_z = -0.1,
                 z_hat_gain = 2.0,
                 seconds_to_keep = 1.0) -> None:
        
        super().__init__(self, Q_mat, R_mat, initX, initP, A, B, H)
        
        # NOTE: Adding all lines from Luenberger observer. Will remove later.
        self._dt = dt
        self._z_0 = None
    ...
    # NOTE: Just resets estimate
    def reset_ic(self):
        self._z_hat = self._z_0
        
    # NOTE: Supposed to be update eq
